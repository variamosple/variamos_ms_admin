...
import { GitHubTokenResolver } from "./GitHubTokenResolver.js";
...
describe("GitHubTokenResolver - Unit Tests", () => {
  ...
  let mockSign: { update: vi.Mock; sign: vi.Mock };
  let dateSpy: vi.SpyInstance;
  ...
  beforeEach(() => {
    ...
    (crypto.createSign as vi.Mock).mockReturnValue(mockSign);
  });
  ...
  describe("resolveGitHubToken", () => {
    it("should return the cached token if it is valid and not close to expiry", async () => {
      mockConfig.getGitHubAppId = vi.fn().mockReturnValue("12345");
      mockConfig.getGitHubPrivateKey = vi.fn().mockReturnValue("mock-key");
    ...
    it("should fetch new token using GitHub App JWT and update cache on miss", async () => {
      ...
      mockConfig.getGitHubPrivateKey = vi
        .fn()
      ...
      expect(mockSign.update).toHaveBeenCalledWith(
      ...
      (axios.get as vi.Mock).mockClear();
      const token2 = await resolver.resolveGitHubToken(
    ...
    it("should fall back to personal github token if app credentials are missing", async () => {
      mockConfig.getGitHubAppId = vi.fn().mockReturnValue(undefined);
      mockConfig.getGitHubPrivateKey = vi.fn().mockReturnValue(undefined);
